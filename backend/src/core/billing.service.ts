...
export class BillingService {
  ...
  /**
   * Inicia trial grátis para um hotel recém-criado
   */
  async startTrial(hotelId: string, days: number = 14): Promise<any> {
    const now = new Date();
    ...
    await this.createBillingLog(hotelId, 'TRIAL_STARTED', `Trial de ${days} dias iniciado. Término: ${trialEnd.toLocaleDateString('pt-BR')}`, {
      days,
  ...
  /**
   * Finaliza trials expirados — converte para pagantes ou suspende
   */
  async processExpiredTrials(): Promise<any> {
    const now = new Date();
    const expiredTrials = await this.prisma.client.hotel.findMany({
      where: {
    ...
    const results = { converted: 0, suspended: 0, notified: 0 };

    for (const hotel of expiredTrials) {
      try {
        // Gerar primeira fatura pós-trial
        const invoice = await this.generateInvoice(hotel.id);

        if (invoice) {
          // Tentar cobrar automaticamente
          const paymentResult = await this.processInvoicePayment(invoice.id);
        ...
        // Se não conseguiu cobrar, avisar e dar mais 3 dias
        const graceEnd = new Date();
        graceEnd.setDate(graceEnd.getDate() + 3);

        await this.prisma.client.hotel.update({
          where: { id: hotel.id },
          data: { nextBillingDate: graceEnd },
        });

        await this.createBillingLog(hotel.id, 'TRIAL_ENDED', 'Trial expirado. Período de cortesia de 3 dias.', {
          trialEndedAt: hotel.trialEndsAt?.toISOString(),
          graceEnd: graceEnd.toISOString(),
        });

        results.notified++;
      } catch (err) {
        this.logger.error(`Erro ao processar trial expirado para ${hotel.id}:`, err);
      }
    }
