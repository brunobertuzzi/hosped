...
export default function GuiaPage() {
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-[#030308]/70 backdrop-blur-2xl border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-[12px] font-bold text-white/70 uppercase tracking-widest">
            ...
            <Link href="/guia" className="text-indigo-400 border-b-2 border-indigo-400 pb-1">
              Guia de Uso
            </Link>
            <a
      ...
      {/* Hero Section */}
      <section className="relative pt-44 pb-20 px-6 min-h-[65vh] flex flex-col items-center justify-center">
        ...
        >
          ...
          <div className="max-w-2xl mx-auto relative group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-indigo-400 group-focus-within:text-purple-300 transition-colors" />
            <input
              type="text"
              placeholder="Pesquisar no guia (ex: reservas, check-in, PIX, governança)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-14 pr-12 py-4 bg-[#080814]/80 border border-white/15 rounded-2xl text-white placeholder:text-white/40 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all text-sm backdrop-blur-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)]"
            />
            {searchQuery && (
              <button
      ...
      <div className="w-full py-6 border-y border-white/10 bg-black/40 backdrop-blur-md overflow-hidden relative z-20">
        <div className="animate-marquee whitespace-nowrap flex items-center gap-12 text-[12px] font-bold text-white/50 uppercase tracking-widest font-mono">
          <span>• GUIA DE USO OFICIAL</span>
          <span>• PASSO A PASSO COMPLETO</span>
          <span>• MAPA DE OCUPAÇÃO & GANTT</span>
          <span>• PIX & CARTÃO AUTOMÁTICOS</span>
            ...
            {guideSections.map((section) => {
              ...
              return (
                <a
                  key={section.id}
                  href={`#${section.id}`}
                  className="flex items-center gap-2.5 px-4 py-3 bg-white/[0.03] border border-white/10 rounded-xl hover:bg-white/[0.08] hover:border-indigo-500/40 hover:scale-105 transition-all text-[12px] font-bold text-white/70 hover:text-white backdrop-blur-xl shadow-[0_4px_15px_rgba(0,0,0,0.4)]"
                >
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-5 mb-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shadow-[0_0_20px_rgba(99,102,241,0.2)]">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-base md:text-lg mt-1 font-normal">
                      {section.subtitle}
                    </p>
                  </div>
                </div>

                <div className="h-px bg-gradient-to-r from-indigo-500/50 via-purple-500/30 to-transparent mb-10" />
                      ...
                      >
                        <div>
                          <div className="flex items-start gap-4 mb-4">
                            <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 text-indigo-400 group-hover:bg-indigo-500/20 group-hover:scale-110 transition-all duration-300">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-white/60 leading-relaxed font-normal mt-2">
                                {topic.desc}
                              </p>
                            </div>
                          </div>
                        ...
                        {topic.tip && (
                          <div className="mt-6 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/25 flex items-start gap-3">
                            <Sparkles className="w-4 h-4 text-indigo-300 shrink-0 mt-0.5" />
                            <p className="text-xs text-indigo-200 font-medium leading-relaxed">
                              <strong className="text-white font-bold">Dica: </strong>
                              {topic.tip}
                            </p>
                          </div>
                        )}
      ...
      {filteredSections.length === 0 && (
        <div className="py-32 px-6 text-center relative z-10">
          ...
          <h3 className="text-2xl font-bold mb-2 text-white">Nenhum resultado encontrado</h3>
          <p className="text-white/50 max-w-md mx-auto">
            Tente buscar por termos como "reserva", "check-in", "financeiro" ou "configurações".
          </p>
          <button
            onClick={() => setSearchQuery('')}
            className="mt-6 px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full text-xs font-bold uppercase tracking-widest transition-all hover:scale-105 shadow-[0_0_20px_rgba(99,102,241,0.4)]"
          >
            Limpar busca
          </button>
        </div>
      ...
      <section className="py-24 px-6 relative z-10">
        <Card3D className="max-w-4xl mx-auto p-12 md:p-16 text-center bg-gradient-to-r from-indigo-950/40 via-purple-950/40 to-indigo-950/40 border border-indigo-500/30">
          <h2 className="text-3xl md:text-5xl font-extrabold mb-4 text-white">
            Ainda tem dúvidas?
          </h2>
          <p className="text-white/70 text-lg mb-10 max-w-2xl mx-auto font-normal">
            Nossa equipe de suporte está pronta para ajudar você e sua equipe.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a
              href={`mailto:${supportEmail}`}
              className="inline-flex items-center gap-3 px-10 py-5 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-400 hover:to-pink-400 text-white font-bold text-[13px] uppercase tracking-widest rounded-full transition-all shadow-[0_0_35px_rgba(99,102,241,0.5)] hover:scale-105"
            >
      ...
      <footer className="border-t border-white/10 py-12 px-6 relative z-10 bg-black/60 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center relative gap-6">
          ...
          <div className="flex items-center gap-8 text-[11px] text-white/50 uppercase tracking-widest font-bold">
            ...
            <Link href="/guia" className="hover:text-white transition-colors text-indigo-400">
              Guia de Uso
            </Link>
          </div>
          <p className="text-[11px] text-white/40 uppercase tracking-widest font-bold text-center md:absolute md:right-0">
